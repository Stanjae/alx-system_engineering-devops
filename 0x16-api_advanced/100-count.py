...
def count_words(subreddit, word_list, word_count=[], page_after=None):
    ...
    if page_after is None:
        custom_url = 'https://www.reddit.com/r/{}/hot.json'.format(subreddit)
        r = get(custom_url, headers=headers, allow_redirects=False)
        ...
        custom_url = ('https://www.reddit.com/r/{}/hot.json?after={}'
               .format(subreddit,
                       page_after))
        r = get(custom_url, headers=headers, allow_redirects=False)
        ...
        if r.status_code == 200:
            ...
            if r.json()['data']['after'] is not None:
                ...
                for key_word in list(set(word_list)):
                    ...
                    if word_count[i] != 0:
                        dict_object[word_list[i]] = (word_count[i] *
                                               word_list.count(word_list[i]))

                for key, value in sorted(dict_object.items(),
